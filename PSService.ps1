...
if ($Setup) {                   # Install the service
  ...
  # Output example-config file, don't replace file if it already exists but instead create config-example.psd1
  if(!(Test-Path "$installDir\config.psd1")) {
    @"
    @{
        Interval = 30
...
"@ | Set-Content -Path "$installDir\config.psd1"
  }
  else {
    @"
    @{
...
"@ | Set-Content -Path "$installDir\config-example.psd1"
  }

  # Create In and Out folders for data matching example config
  if (!(Test-Path "$installDir\In")) {
    New-Item -ItemType directory -Path "$installDir\In" | Out-Null
  }
  if (!(Test-Path "$installDir\Out")) {
    New-Item -ItemType directory -Path "$installDir\Out" | Out-Null
  }
